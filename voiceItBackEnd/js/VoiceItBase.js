...
function voiceItModule (config, server) {
  var main = this;
  ...
  var faceOtherWayCounter = 0;


  //TODO: get the phrases for an account dynamically, and send to the front end
  this.getPhrases = function (lang) {
    myVoiceIt.getAllPhrases({
  ...
  //this.getPhrases(config.contentLanguage);
      ...
      case 0:
        ...
        if ((timeNew - testTimer) > 750) {
          if (face >= 0.35) {
            ...
            faceOtherWayCounter++;
            if (faceOtherWayCounter > 2){
              livTries++;
              doingLiveness = false;
              if (livTries > MAX_TRIES) {
                io.emit('stopRecording', 0);
  ...
  io.on('connection', function(socket) {
    ...
    socket.on('requestPhrases', function (){
      socket.emit('requestPhrases', { phrase: main.currPhrase });
    });
    socket.on('requestAllEnrollmentDetails', function(request) {
      myVoiceIt.getAllEnrollmentsForUser({
        ...
        io.emit('allEnrollmentNeeded', obj);
        main.emit('result', {response: jsonResponse, type: "getAllEnrollments"});
      });
      socket1Id = socket.id;
    });

    socket.on('requestFaceEnrollmentDetails', function(request) {
      myVoiceIt.getFaceEnrollmentsForUser({
        ...
        io.emit('faceEnrollmentNeeded', obj);
      ...
      if (doingLiveness) {
        ...
        if (passed.value) {
          ...
          if (passedTests < numTests) {
            ...
            if (oldTest < 3) {
              if (successTimeStamps.length < numTests) {
                successTimeStamps.push(timeStamps[timeStamps.length - 1]+400);
              }
        ...
        if (checkForFaceStraight) {
          var a = Date.now();
          if ((face.rotationY < 0.2 && face.rotationY > -0.2 && face.rotationX < 0.20) || (a - timePassed > 1000)) {
            testTimer = Date.now();
            if (successTimeStamps.length < numTests) {
              successTimeStamps.push(timeStamps[timeStamps.length - 1]);
            }
    ...
    socket.on('terminateVoiceItObj', function() {
      doingLiveness = false;
      socket.disconnect(true);
    });

    socket.on('terminateLiveness', function() {
      doingLiveness = false;
      socket.disconnect(true);
    });
  });
